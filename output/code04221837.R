...
dtrain=xgb.DMatrix(cbind(data.matrix(aaa1),cate_dou_na),label=train$gpa[which(!is.na(train$gpa))])
...
xgb.cv(nfold=10,data=dtrain,params = params,nround=3000)
model8=xgb.train(data=dtrain,params=params,nrounds=100)
...
tar=read_csv('prediction.csv')
tar$gpa=sub
write_csv(tar,'submissionhope.csv')
