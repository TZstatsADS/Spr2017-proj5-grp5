...
tar$materialHardship=sub
write_csv(tar,'submissionhard.csv')


### Kai ge
#my.text <- "^([a-z]{1,3}5)"
#my.text <- "^(k5|o5|n5|hv5|p5)"
my.text <- "^(m)"
...
# dtrain=xgb.DMatrix(cbind(data.matrix(aaa1),cate_dou_na),label=train$materialHardship[which(!is.na(train$materialHardship))])
# dtest=xgb.DMatrix(cbind(data.matrix(aaa_pre1),cate_pred_dou_na))
# dtrain=xgb.DMatrix(data.matrix(aaa1),label=train$grit[which(!is.na(train$grit))])
# dtest=xgb.DMatrix(data.matrix(aaa_pre1))
#dtest=xgb.DMatrix(new_features_test)
params=list(
        objective='reg:linear',
        subsample=0.9,
...
tar=read_csv('prediction.csv')
tar$materialHardship=sub
write_csv(tar,'submission_hard.csv')

dou_mom <- read.csv("submissionhard.csv")
cate_mom <- read.csv("submission_hard.csv")
mom_netrual <- 0.4 * dou_mom$materialHardship + 0.6*cate_mom$materialHardship
tar=read_csv('prediction.csv')
tar$materialHardship=mom_netrual
write_csv(tar,'submission_hnetral1.csv')
